...
function getBanner(section, course_slug, slug) {
  return `https://raw.githubusercontent.com/shovankarna/portfolio/master/public/static/courses/${course_slug}/${section}/${slug}/banner.png`;
}
...
function getBody(course_slug, slug) {
  ...
  body = body.replace(
    /\]\(\/static\/courses/g,
    '](https://raw.githubusercontent.com/shovankarna/portfolio/master/public/static/courses',
  );

  // Replace static links
  body = body.replace(
    new RegExp(`]\\(/courses/${course_slug}`, 'g'),
    `](https://shovankarna.com/courses/${course_slug}`,
  );
...
(async function main() {
  ...
  for (const [, { name, section, slug }] of articles.entries()) {
    ...
    const canonical_url = `https://github.com/shovankarna/${course_slug}#${slug}`;
